...
<html lang="en">
  ...
  <div>
<?php
require_once('productsDB.php'); // Include database connection

ini_set('display_errors', 1);
error_reporting(E_ALL);

// Set User ID (this should ideally come from session or authentication)
$UserID = 1;
...
if ($stmt) {
    ...
    $Fname = $user ? htmlspecialchars($user['first_name']) : "";
    $Lname = $user ? htmlspecialchars($user['last_name']) : "";
    $Email = $user ? htmlspecialchars($user['email']) : "";
    $Address = $user ? htmlspecialchars($user['city']) : "";
...
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST["save"])) {
    ...
    if (!empty($Fname) && !empty($Lname) && !empty($Email) && !empty($Address)) {
        $sql = "UPDATE tbl_users SET first_name = ?, last_name = ?, email = ?, city = ? WHERE userID = ?";
        $stmtupdate = $mysqli->prepare($sql);

        if ($stmtupdate) {
            $stmtupdate->bind_param("ssssi", $Fname, $Lname, $Email, $Address, $UserID);
            if ($stmtupdate->execute()) {
                echo "<script>alert('Update successful'); window.location.href='userAccount.php';</script>";
            } else {
                echo "<script>alert('Update failed: " . $stmtupdate->error . "');</script>";
            }
            $stmtupdate->close();
        } else {
            echo "<script>alert('Update query preparation failed: " . $mysqli->error . "');</script>";
        }
    } else {
